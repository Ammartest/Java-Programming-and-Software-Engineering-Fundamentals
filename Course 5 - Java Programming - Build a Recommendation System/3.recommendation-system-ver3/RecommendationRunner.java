...
public class RecommendationRunner implements Recommender {
    ...
    public void printRecommendationsFor(String webRaterID) {
        ...
        if (recommendations.size() == 0) {
            ...
            ArrayList<String> movieToBeRated = getItemsToRate();
            ArrayList<Rating> outID = new ArrayList<>();
            int count = 0;
            for (int i = 0; outID.size() + count != recommendations.size() && outID.size() < 10; i++) {
                if (!movieToBeRated.contains(recommendations.get(i).getItem())) {
                    outID.add(recommendations.get(i));
                } else {
                    count++;
                }
            }
            ...
            System.out.println("  background-color: #001D6E;");
